...
export const commands: CommandDoc[] = [
  {
    ...
    flags: [
      ...
      { flag: "--compact", description: "Compact conversation history for agents that had changes applied (clears stale context)", type: "boolean" },
      { flag: "--compact-tags", description: "Only compact agents matching these tags (comma-separated, AND logic)", type: "string" },
      { flag: "--compact-match", description: "Only compact agents matching glob pattern", type: "string" },
      { flag: "--recalibrate", description: "Send a calibration message to agents that had changes applied", type: "boolean" },
      { flag: "--recalibrate-message", description: "Custom calibration message (overrides default)", type: "string" },
      { flag: "--recalibrate-tags", description: "Only recalibrate agents matching these tags (comma-separated, AND logic)", type: "string" },
      { flag: "--recalibrate-match", description: "Only recalibrate agents matching glob pattern", type: "string" },
      { flag: "--no-wait", description: "Fire-and-forget recalibration (don't wait for responses)", type: "boolean" },
    ],
    examples: [
      ...
      { title: "Compact + recalibrate", code: "lettactl apply -f fleet.yaml --compact --recalibrate" },
      { title: "Deploy and recalibrate", code: "lettactl apply -f fleet.yaml --recalibrate" },
